...
```
Una vez hemos dispuesto las tablas, procedemos a unir los datos.
...
datatable(union)
```
Con esta información podemos ver las provincia con proporción mayor de infectados y de entre ellos si prevalecen hombres o mujeres.

```{r}
ggplot(data = union, aes(x = Provincia, y = Año))+
